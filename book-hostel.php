...
$query="insert into  registration(roomno,seater,feespm,foodstatus,stayfrom,duration,firstName,middleName,lastName,gender,contactno,emailid,egycontactno,guardianName,guardianRelation,guardianContactno,corresAddress,corresCIty,corresState,corresPincode,pmntAddress,pmntCity,pmnatetState,pmntPincode) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
$stmt = $mysqli->prepare($query);
$rc=$stmt->bind_param('iiiisissssisississsisssi',$roomno,$seater,$feespm,$foodstatus,$stayfrom,$duration,$fname,$mname,$lname,$gender,$contactno,$emailid,$emcntno,$gurname,$gurrelation,$gurcntno,$caddress,$ccity,$cstate,$cpincode,$paddress,$pcity,$pstate,$ppincode);
$stmt->execute();
...
<input type="radio" value="1" id="x" name="foodstatus"> With Food(Rs 2000.00 Per Month Extra)
</div>
</div>	
...
<div class="col-sm-8">
<select name="duration" id="duration" class="form-control" onchange="total()">
...
function total() {
  ...
  var y = document.getElementsByName("foodstatus");
  var z = document.getElementById("fpm").value;
	for(i = 0; i < y.length; i++) { 
		if(y[i].checked) 
		var f =y[i].value; 
	} 
  document.getElementById("ta").innerHTML =  z*x+f*2000;
  console.log(x,z,f)
}
</script>
<div class="form-group">
